...
[Combine](combine.md), Subtract, [Intersect](intersect.md) and [Subtract and Replace](subtract-and-replace.md) are all performed by one Boolean object -- the toolbar button creates it with Subtract already selected, and you can switch to any of the other three at any time from the **Operation** icon row at the top of the Properties panel.
