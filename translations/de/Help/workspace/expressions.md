...
Viele Parameter in MatterCAD akzeptieren mathematische Ausdrücke anstelle einfacher Zahlen. Dies ermöglicht parametrisches Konstruieren, bei dem die Änderung eines Werts automatisch zugehörige Maße aktualisiert.
...
- Ausdrücke werden in jedem Feld unterstützt, das im Code `DoubleOrExpression`, `IntOrExpression` oder `StringOrExpression` anzeigt – in der Praxis akzeptieren die meisten numerischen Felder in Konstruktionswerkzeugen sie
- Verwenden Sie Ausdrücke, um Beziehungen zwischen Parametern herzustellen – setzen Sie beispielsweise den Durchmesser einer Bohrung auf `outer_diameter - 4`, damit sie immer 2 mm starke Wandungen hat
- Ausdrücke werden automatisch aktualisiert, wenn sich referenzierte Werte ändern
...
- [Komponenten](components.md) – Wiederverwendbare parametrisierte Konstruktionen erstellen
- [Variablenblatt](variable-sheet.md) – Gemeinsam genutzte Werte und Formeln für eine Konstruktion speichern
